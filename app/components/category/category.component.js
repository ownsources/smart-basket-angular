"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var category_data_service_1 = require("../../shared/services/category-data.service");
var product_data_service_1 = require("../../shared/services/product-data.service");
var CategorysComponent = (function () {
    function CategorysComponent(categoryDataService, productDataService) {
        this.categoryDataService = categoryDataService;
        this.productDataService = productDataService;
        this.categorys = [];
    }
    CategorysComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.categoryDataService.getCategory()
            .subscribe(function (categorys) { return _this.categorys = categorys; });
    };
    CategorysComponent.prototype.create = function (name) {
        this.categoryDataService.createCategory(name);
    };
    CategorysComponent.prototype.delete = function (category) {
        this.categoryDataService.deleteCategory(category);
    };
    CategorysComponent.prototype.update = function (category) {
        this.categoryDataService.updateCategory(category);
    };
    CategorysComponent.prototype.select = function (category) {
        this.productDataService.filterProduct(category);
    };
    CategorysComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
            selector: 'categorys',
            templateUrl: 'category.component.html',
            styleUrls: ['../../styles/list.component.css']
        }),
        core_1.Injectable(), 
        __metadata('design:paramtypes', [category_data_service_1.CategoryDataService, product_data_service_1.ProductDataService])
    ], CategorysComponent);
    return CategorysComponent;
}());
exports.CategorysComponent = CategorysComponent;
//# sourceMappingURL=category.component.js.map